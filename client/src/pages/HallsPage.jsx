        ...
        <div className="space-y-8 animate-slide-up">
            <div className="glass p-8 rounded-3xl border-l-[6px] border-l-amber-500 shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-600 ml-16">Configure your physical venues and layout dimensions.</p>
                </div>
                <div className="bg-white border border-slate-300 px-6 py-4 rounded-2xl md:ml-0 ml-16 w-full md:w-auto text-center shadow-lg shadow-black/50">
                    <p className="text-sm text-slate-600 font-bold uppercase tracking-wider mb-1">Total Venue Capacity</p>
                ...
                <div className="lg:col-span-1">
                    <div className="glass p-6 rounded-3xl shadow-xl sticky top-24">
                        ...
                        <form onSubmit={handleAdd} className="space-y-5">
                            <div>
                                <label className="block text-xs font-semibold text-slate-600 mb-1 ml-1 uppercase">Hall Name / Number</label>
                                <input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required placeholder="Main Hall A"
                                    className="w-full bg-white/80 border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:border-amber-500 transition-colors font-bold text-slate-900 tracking-wide" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-semibold text-slate-600 mb-1 ml-1 uppercase">Rows</label>
                                    <input type="number" min="1" max="50" value={form.rows} onChange={e => setForm({ ...form, rows: e.target.value })} required
                                        className="w-full bg-white/80 border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:border-amber-500 transition-colors text-center font-mono text-lg" />
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold text-slate-600 mb-1 ml-1 uppercase">Columns</label>
                                    <input type="number" min="1" max="50" value={form.cols} onChange={e => setForm({ ...form, cols: e.target.value })} required
                                        className="w-full bg-white/80 border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:border-amber-500 transition-colors text-center font-mono text-lg" />
                                </div>
                            </div>

                            <div className="bg-white border border-slate-200 rounded-xl p-4 flex justify-between items-center hidden">
                                <span className="text-xs font-bold text-slate-600 uppercase tracking-wider">Preview Capacity</span>
                                <span className="text-xl font-black text-amber-400">{(form.rows || 0) * (form.cols || 0)}</span>
                            </div>

                            <button disabled={loading} type="submit" className="w-full py-3.5 mt-2 rounded-xl font-bold bg-gradient-to-r from-amber-500 to-amber-600 hover:shadow-lg hover:shadow-amber-500/20 hover:-translate-y-1 transition-all flex items-center justify-center gap-2 text-slate-900 shadow-md shadow-amber-900/20">
                ...
                <div className="lg:col-span-3">
                    {loading && halls.length === 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {[1, 2, 3].map(i => <div key={i} className="h-64 rounded-3xl shimmer border border-slate-200" />)}
                        </div>
                    ) : halls.length === 0 ? (
                        <div className="glass p-16 rounded-3xl text-center border-dashed border-2 border-slate-300 h-full flex flex-col items-center justify-center">
                            <div className="w-20 h-20 rounded-full bg-slate-200 mx-auto mb-6 flex items-center justify-center text-slate-600 shadow-inner"><AlertCircle size={40} /></div>
                            <h3 className="font-bold text-2xl text-slate-900 mb-2">No Exam Halls Found</h3>
                            <p className="text-slate-600">Add physical venues and specify their dimensions (rows × columns) to begin allocating students.</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {halls.map((h, idx) => (
                                <div key={h.id} className="glass p-6 rounded-3xl relative group hover:border-amber-500/50 transition-colors animate-slide-up hover:-translate-y-1 duration-300" style={{ animationDelay: `${idx * 0.05}s` }}>
                                    ...
                                    <div className="flex justify-between items-start mb-6 border-b border-slate-200 pb-4">
                                        <div>
                                            <h3 className="font-black text-xl text-slate-900 leading-tight mb-1">{h.name}</h3>
                                            <p className="text-sm text-slate-600 font-mono tracking-wider">{h.rows} ROWS × {h.cols} COLS</p>
                                        </div>
                                        <button onClick={() => handleDelete(h.id, h.name)} className="text-slate-600 hover:text-red-400 p-2.5 rounded-xl hover:bg-red-500/10 transition-colors" title="Delete Hall">
                                            <Trash2 size={18} />
                                        </button>
                                    </div>

                                    {/* Mini Grid Preview */}
                                    <div className="h-[200px] w-full bg-white/80 rounded-2xl flex items-center justify-center p-4 border border-slate-200 mb-6 relative overflow-hidden group-hover:border-slate-300 transition-colors">
                                    ...
                                    <div className="flex justify-between items-end">
                                        <div>
                                            <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest mb-1">Total Capacity</p>
                                            <p className="text-3xl font-black text-amber-400 leading-none">{h.capacity}</p>
                                        </div>
                                        <div className="w-8 h-8 rounded-full bg-slate-200 flex items-center justify-center text-slate-600 text-xs font-bold border border-slate-300">🪑</div>
                                    </div>
                                </div>
                            ))}
                        </div>
