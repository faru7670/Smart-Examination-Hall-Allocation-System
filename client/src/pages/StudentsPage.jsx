        ...
        <div className="space-y-8 animate-slide-up">
            <div className="glass p-8 rounded-3xl border-l-[6px] border-l-indigo-500 shadow-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-600 ml-16">Enroll students individually or upload a CSV file.</p>
                </div>
                <div className="bg-white border border-slate-300 px-6 py-4 rounded-2xl md:ml-0 ml-16 w-full md:w-auto text-center shadow-lg shadow-black/50">
                    <p className="text-sm text-slate-600 font-bold uppercase tracking-wider mb-1">Total Enrolled</p>
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-6">
                    <div className="glass p-6 rounded-3xl shadow-xl">
                        ...
                        <div className="border-2 border-dashed border-slate-300 rounded-2xl p-6 text-center hover:border-emerald-500/50 hover:bg-emerald-500/5 transition-colors cursor-pointer"
                            onClick={() => fileInputRef.current.click()}>
                            <Upload className="mx-auto h-10 w-10 text-slate-600 mb-3" />
                            <p className="text-sm font-semibold text-slate-900 mb-1">Click to upload CSV</p>
                            <p className="text-xs text-slate-600">Format: ID, Name, Subject</p>
                        </div>
                    ...
                    <div className="glass p-6 rounded-3xl shadow-xl">
                        ...
                        <form onSubmit={handleAddSingle} className="space-y-4">
                            <div>
                                <label className="block text-xs font-semibold text-slate-600 mb-1 ml-1 uppercase">Student ID / Roll No</label>
                                <input type="text" value={form.student_id} onChange={e => setForm({ ...form, student_id: e.target.value })} required placeholder="14041"
                                    className="w-full bg-white/80 border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:border-indigo-500 transition-colors uppercase" />
                            </div>
                            <div>
                            ...
                            <div>
                                <label className="block text-xs font-semibold text-slate-600 mb-1 ml-1 uppercase">Subject Code</label>
                                <input type="text" value={form.subject_code} onChange={e => setForm({ ...form, subject_code: e.target.value })} required placeholder="CS101"
                                    className="w-full bg-white/80 border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:border-indigo-500 transition-colors uppercase" />
                            </div>
                            <button disabled={loading} type="submit" className="w-full py-3 mt-4 rounded-xl font-bold bg-gradient-to-r from-indigo-500 to-indigo-600 hover:shadow-lg hover:shadow-indigo-500/20 hover:-translate-y-1 transition-all flex items-center justify-center gap-2 text-slate-900">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-between items-center glass p-4 rounded-3xl relative z-10">
                        <div className="relative w-full sm:w-96">
                            ...
                            <input type="text" placeholder="Search by ID, Name, or Subject..." value={search} onChange={e => setSearch(e.target.value)}
                                className="w-full bg-white/80 border border-slate-200 rounded-xl pl-12 pr-4 py-2.5 focus:outline-none focus:border-indigo-500 transition-colors" />
                        </div>
                        {students.length > 0 && (
                            <button onClick={handleClearAll} disabled={loading} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-red-500/10 text-red-500 font-bold hover:bg-red-500 hover:text-slate-900 transition-colors w-full sm:w-auto justify-center whitespace-nowrap border-red-500/20 border">
                                <Trash2 size={16} /> Clear All
                            </button>
                        )}
                    </div>

                    <div className="glass rounded-3xl shadow-xl overflow-hidden relative border border-slate-200">
                        <div className="overflow-x-auto max-h-[700px] overflow-y-auto no-scrollbar">
                            <table className="w-full text-left border-collapse">
                                <thead className="bg-white/80 sticky top-0 z-10 backdrop-blur-md">
                                    <tr>
                                        <th className="p-4 text-xs font-semibold text-slate-600 uppercase tracking-widest border-b border-slate-200">Student ID</th>
                                        <th className="p-4 text-xs font-semibold text-slate-600 uppercase tracking-widest border-b border-slate-200">Name</th>
                                        <th className="p-4 text-xs font-semibold text-slate-600 uppercase tracking-widest border-b border-slate-200">Subject</th>
                                        <th className="p-4 text-xs font-semibold text-slate-600 uppercase tracking-widest border-b border-slate-200 text-right">Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {loading && students.length === 0 ? (
                                        [1, 2, 3, 4, 5].map(i => (
                                            <tr key={i} className="border-b border-slate-200/50"><td colSpan={4} className="p-4"><div className="h-4 shimmer rounded-full w-full opacity-30" /></td></tr>
                                        ))
                                    ) : filtered.length === 0 ? (
                                        <tr><td colSpan="4" className="p-16 text-center text-slate-600">
                                            <AlertCircle size={40} className="mx-auto mb-4 opacity-50" />
                                            <p className="font-bold text-lg text-slate-900 mb-1">No students found</p>
                                            <p className="text-sm">Time to upload your roster!</p>
                                        </td></tr>
                                    ) : (
                                        filtered.map(s => (
                                            <tr key={s.id} className="border-b border-slate-200/50 hover:bg-slate-200/30 transition-colors">
                                                <td className="p-4 font-mono font-bold text-slate-900 max-w-[120px] truncate" title={s.student_id}>{s.student_id}</td>
                                                ...
                                                <td className="p-4 text-right">
                                                    <button onClick={() => handleDelete(s.id)} className="text-slate-600 hover:text-red-400 p-2 hover:bg-red-500/10 rounded-lg transition-colors inline-block" title="Delete">
                                                        <Trash2 size={16} />
                                                    </button>
                                                </td>
                                            </tr>
                        ...
                        <div className="bg-white border-t border-slate-200 p-4 text-center text-xs font-bold text-slate-600 uppercase tracking-widest">
