...
export default function DashboardPage({ collegeCode }) {
    ...
    if (loading) return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-slide-up">
            {[1, 2, 3, 4].map(i => <div key={i} className="h-32 glass rounded-3xl shimmer border border-slate-700/50" />)}
        </div>
    )
        ...
        <div className="space-y-8 animate-slide-up">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="glass p-6 rounded-3xl relative overflow-hidden group hover:-translate-y-1 transition-transform border-t border-t-indigo-500/30">
                    ...
                    <h3 className="text-4xl font-black text-white">{stats.students}</h3>
                </div>
                ...
                <div className="glass p-6 rounded-3xl relative overflow-hidden group hover:-translate-y-1 transition-transform border-t border-t-amber-500/30">
                    ...
                    <div className="flex justify-between items-start mb-4">
                        ...
                        <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Venues</span>
                    </div>
                    <h3 className="text-4xl font-black text-white">{stats.halls} <span className="text-lg text-slate-500 font-mono">({stats.totalCapacity} seats)</span></h3>
                </div>
                ...
                <div className="glass p-6 rounded-3xl relative overflow-hidden group hover:-translate-y-1 transition-transform border-t border-t-emerald-500/30">
                    ...
                    <div className="flex justify-between items-start mb-4">
                        ...
                        <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Allocated</span>
                    </div>
            ...
            <div className="grid lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 glass p-6 rounded-3xl border border-slate-800 shadow-xl">
                    ...
                    <div className="h-72">
                        {Object.keys(hallDist).length > 0 ? (
                            ...
                            <div className="h-full flex items-center justify-center text-slate-500 font-mono text-sm border-2 border-dashed border-slate-800 rounded-2xl">No halls configured</div>
                        )}
                    </div>
                </div>

                <div className="lg:col-span-1 glass p-6 rounded-3xl border border-slate-800 shadow-xl">
                    <h3 className="font-bold text-lg mb-6 flex items-center gap-2 text-white">
                        ...
                        {Object.keys(subDist).length > 0 ? (
                            <>
                                ...
                                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none -ml-24">
                                    <span className="text-3xl font-black text-white">{Object.keys(subDist).length}</span>
                                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Subjects</span>
                                </div>
                            </>
                        ) : (
                            <div className="h-full w-full flex items-center justify-center text-slate-500 font-mono text-sm border-2 border-dashed border-slate-800 rounded-2xl">No subjects found</div>
                        )}
                    </div>
                </div>
            </div>
            ...
            {stats.students === 0 && stats.halls === 0 && (
                <div className="glass p-8 rounded-3xl border border-slate-800 mt-8 relative overflow-hidden border-t-2 border-t-cyan-500/50">
                    <div className="absolute right-0 top-0 w-64 h-64 bg-cyan-500/5 rounded-full blur-[100px]" />
                    <h3 className="text-2xl font-black mb-4 text-white">🚀 Initial Setup Guide</h3>
                    <div className="grid md:grid-cols-3 gap-6 relative z-10">
                        <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-700/50 hover:bg-slate-800/50 transition-colors">
                            <span className="bg-indigo-500 text-white w-8 h-8 rounded-lg flex items-center justify-center font-bold mb-4 shadow-lg shadow-indigo-500/30">1</span>
                            <h4 className="font-bold text-white mb-2">Upload Students</h4>
                            <p className="text-sm text-slate-400">Head to the Students tab and upload your `.csv` file containing Roll Numbers and Subject priorities.</p>
                        </div>
                        <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-700/50 hover:bg-slate-800/50 transition-colors">
                            <span className="bg-amber-500 text-white w-8 h-8 rounded-lg flex items-center justify-center font-bold mb-4 shadow-lg shadow-amber-500/30">2</span>
                            <h4 className="font-bold text-white mb-2">Configure Halls</h4>
                            <p className="text-sm text-slate-400">Go to the Halls tab and register physical exam venues by setting exact row and column dimensions.</p>
                        </div>
                        <div className="bg-slate-900/50 p-6 rounded-2xl border border-slate-700/50 hover:bg-slate-800/50 transition-colors">
                            <span className="bg-emerald-500 text-white w-8 h-8 rounded-lg flex items-center justify-center font-bold mb-4 shadow-lg shadow-emerald-500/30">3</span>
                            <h4 className="font-bold text-white mb-2">Run Allocation</h4>
                            <p className="text-sm text-slate-400">In the Allocation tab, securely execute the matrix algorithm to seat students and completely avoid adjacent cheating.</p>
                        </div>
                    </div>
                </div>
            )}
